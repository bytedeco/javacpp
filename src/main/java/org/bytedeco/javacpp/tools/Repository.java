...
public class Repository {
    ...
    public Path getPath(Coordinates coordinates) {
        ...
        if (Files.exists(dst)) {
          if (src_time != dst.toFile().lastModified()) {
            try {
              delete(dst);
            } catch (Exception ex) {
        ...
        if (!Files.exists(dst)) {
            ...
            dst.toFile().setLastModified(src_time);
        }
        return dst;
    }

    // Jars store links as files containing a target. This heuristic figures
    // if a file is actually a link and reads it's target.
    private static String getLinkTarget(Path path, String platform) throws IOException {
    ...
    private static void expand(final Path src, final Path dst, final String classifier)
            throws IOException {
